...
from twmacl.config import Phase3Config
...
def _build_policies(config: Phase3Config, obs_dim: int, seed: int) -> list[LinearGaussianPolicy]:
...
def _run_episode(
    *,
    ...
    for step in range(episode_length):
        ...
        base_means = np.array([policy.compute_mean(obs) for policy, obs in zip(policies, observations)])
        peer_means = normalized_trust @ base_means
        hybrid_means = (1.0 - beta) * base_means + beta * peer_means
        action_results: list[ActionResult] = [
            policy.act(observation, deterministic=(mode == "eval"), mean_override=hybrid_means[i])
            for i, (policy, observation) in enumerate(zip(policies, observations, strict=True))
        ]
        ...
        for agent_idx, policy in enumerate(policies):
            ...
            if mode == "train":
                update_info = policy.update(
                    observations[agent_idx], action_results[agent_idx], float(agent_returns[agent_idx]), gradient_scale=(1.0 - config.learning.imitation_beta)
                )
...
def run_phase3(config: Phase3Config) -> None:
