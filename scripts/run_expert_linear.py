...
def main():
    config_dict = {
        ...
        "experiment": {
            "num_seeds": 5,
            "base_seed": 2000,
            "output_root": "outputs/phase5_expert_linear"
        }
    }
    
    config_path = Path("configs/phase5_expert_linear.yaml")
    config_path.parent.mkdir(exist_ok=True)
    with open(config_path, "w") as f:
        yaml.dump(config_dict, f)
        
    print("Running Phase 5 Expert Experiment with Linear Normalization...")
    t0 = time.time()
    ...
    # Analyze
    ablation_dir = Path("outputs/phase5_expert_linear")
    train_steps_list = []
    ...
    print(f"--- Phase 5 Results (Linear) ---")
